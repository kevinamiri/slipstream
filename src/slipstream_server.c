...
ssize_t server_decode(void* slot_p, void* callback_ctx, unsigned char** dest_buf, const unsigned char* src_buf, size_t src_buf_len, struct sockaddr_storage *peer_addr, struct sockaddr_storage *local_addr) {
    ...
    *dest_buf = malloc(src_buf_len);
    ...
    if (question->class != CLASS_IN) {
        // Only IN class questions are supported by slipstream.
        slot->error = RCODE_REFUSED;
        return 0;
    }
...
void* slipstream_server_poller(void* arg) {
    ...
    while (1) {
        ...
        if (fds.revents & POLLNVAL) {
            DBG_PRINTF("poll: file descriptor %d was closed", fds.fd);
            break;
        }
    ...
    case picoquic_callback_prepare_to_send:
        ...
        if (stream_ctx == NULL) {
            ...
            if (length_to_read == 0) {
                ...
                if (bytes_read < 0 && (errno == EAGAIN || (EWOULDBLOCK != EAGAIN && errno == EWOULDBLOCK))) {
